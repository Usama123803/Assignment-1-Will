...
class VehicleUpdate extends BaseController
{
    public function updatemakes()
    {
        try {
    ...
    public function updateyears()
    {
        ...
        $db->table('vehicleyears')->emptyTable('vehicleyears');
        $vyears = $db->table('vehicleyears');
        $endyear = date("Y");
        $endyear = $endyear+1;
        for($firstyear = 1960; $firstyear <= $endyear; $firstyear++)
        {
            $vyears->insert(['years'=>$firstyear]);
        }
